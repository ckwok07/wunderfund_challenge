...
class PredictionModel(nn.Module):
    ...
    def __init__(self):
        ...
        # We now use 6 × 32-dim blocks:
        #   1) raw
        #   2) delta_scaled
        #   3) accel_scaled
        #   4) rolling-normalized raw
        #   5) volatility of delta
        #   6) time-since-last-jump (per-feature, normalized)
        self.raw_dim = 32
        self.input_dim = 32 * 6   # 192
        ...
        # windows / thresholds for extra features
        self.raw_window = 16
        self.delta_window = 16
        self.jump_max = 32
        self.jump_factor = 1.0  # jump threshold = jump_factor * delta_std

        # Online state
        self.current_seq_ix = None
        self.sequence_history = []   # list of 192-dim feature vectors
        self.last_state = None       # last raw state (32,)
        self.last_delta = None       # last delta (32,)
        self.raw_buffer = []         # rolling buffer of raw states
        self.delta_buffer = []       # rolling buffer of deltas
        self.ts_since_jump = None    # per-feature counter

        # LSTM takes 192-dim input
        self.lstm = nn.LSTM(
    ...
    def forward(self, x):
        """
        x: (batch_size, seq_len, input_dim=192)
    ...
    def predict(self, dp: DataPoint):
        ...
        # ---- rolling buffers for extra features ----
        # raw buffer for rolling-normalized raw
        self.raw_buffer.append(raw_state)
        if len(self.raw_buffer) > self.raw_window:
            self.raw_buffer = self.raw_buffer[-self.raw_window:]
        raw_stack = np.stack(self.raw_buffer, axis=0)  # (k, 32)
        raw_mean = raw_stack.mean(axis=0)
        raw_std = np.clip(raw_stack.std(axis=0), 1e-6, None)
        norm_raw = (raw_state - raw_mean) / raw_std        # (32,)

        # delta buffer for volatility + ts_since_jump
        self.delta_buffer.append(delta)
        if len(self.delta_buffer) > self.delta_window:
            self.delta_buffer = self.delta_buffer[-self.delta_window:]
        delta_stack = np.stack(self.delta_buffer, axis=0)  # (k, 32)
        vol_delta = delta_stack.std(axis=0)                # (32,)

        # time-since-last-jump (per feature)
        if self.ts_since_jump is None:
            self.ts_since_jump = np.zeros_like(raw_state, dtype=np.float32)

        if self.scale_delta is not None:
            thresh_vec = self.jump_factor * self.scale_delta
        else:
            # fallback: use current rolling std as proxy scale
            rolling_std = np.clip(delta_stack.std(axis=0), 1e-6, None)
            thresh_vec = self.jump_factor * rolling_std

        jump_mask = np.abs(delta) > thresh_vec
        # reset where big jump, increment otherwise (capped)
        self.ts_since_jump[jump_mask] = 0.0
        self.ts_since_jump[~jump_mask] = np.minimum(
            self.ts_since_jump[~jump_mask] + 1.0,
            float(self.jump_max)
        )
        ts_norm = self.ts_since_jump / float(self.jump_max)  # (32,)
        ...
        # Build 192-dim input:
        # [raw, delta_scaled, accel_scaled, norm_raw, vol_delta, ts_norm]
        feat = np.concatenate(
            [raw_state, delta_scaled, accel_scaled, norm_raw, vol_delta, ts_norm],
            axis=0
        )  # (192,)
        ...
        # Apply bias correction if available (on delta space)
...
def build_features_and_targets(df: pd.DataFrame, feature_cols):
    ...
    Also computes and saves per-feature std for delta and accel (for scaling).
    """
    ...
    # First pass: compute delta & accel per sequence
    ...
    # global std for scaling delta & accel
    ...
    # Second pass: compute extra features per sequence
    norm_raw_all = np.zeros_like(raw_all, dtype=np.float32)
    vol_all = np.zeros_like(raw_all, dtype=np.float32)
    ts_all = np.zeros_like(raw_all, dtype=np.float32)

    raw_window = 16
    delta_window = 16
    jump_max = 32
    jump_factor = 1.0
    threshold_vec = jump_factor * delta_std  # (32,)

    for seq_ix, df_seq in df.groupby("seq_ix"):
        idx = df_seq.index.to_numpy()
        raw_seq = raw_all[idx]       # (T, 32)
        delta_seq = deltas_all[idx]  # (T, 32)
        T = raw_seq.shape[0]

        norm_raw_seq = np.zeros_like(raw_seq, dtype=np.float32)
        vol_seq = np.zeros_like(raw_seq, dtype=np.float32)
        ts_seq = np.zeros_like(raw_seq, dtype=np.float32)

        raw_buffer = []
        delta_buffer = []
        ts = np.zeros(dim, dtype=np.float32)
        ...
        for t in range(T):
            ...
            # rolling-normalized raw
            raw_buffer.append(x)
            if len(raw_buffer) > raw_window:
                raw_buffer = raw_buffer[-raw_window:]
            raw_stack = np.stack(raw_buffer, axis=0)
            r_mean = raw_stack.mean(axis=0)
            r_std = np.clip(raw_stack.std(axis=0), 1e-6, None)
            norm_raw_seq[t] = (x - r_mean) / r_std

            # volatility of delta
            delta_buffer.append(d)
            if len(delta_buffer) > delta_window:
                delta_buffer = delta_buffer[-delta_window:]
            d_stack = np.stack(delta_buffer, axis=0)
            vol_seq[t] = d_stack.std(axis=0)

            # time-since-last-jump (per feature)
            jump_mask = np.abs(d) > threshold_vec
            ts[jump_mask] = 0.0
            ts[~jump_mask] = np.minimum(ts[~jump_mask] + 1.0, float(jump_max))
            ts_seq[t] = ts / float(jump_max)

        norm_raw_all[idx] = norm_raw_seq
        vol_all[idx] = vol_seq
        ts_all[idx] = ts_seq
    ...
    # concat to 192-dim features:
    # [raw, delta_scaled, accel_scaled, norm_raw, vol_delta, ts_since_jump]
    features_all = np.concatenate(
        [raw_all, delta_scaled, accel_scaled, norm_raw_all, vol_all, ts_all],
        axis=1
    )  # (N, 192)
...
def train_model(
    ...
    """
    Train the model on [raw, delta_scaled, accel_scaled,
                        norm_raw, vol_delta, ts_since_jump]
    windows to predict next delta.
