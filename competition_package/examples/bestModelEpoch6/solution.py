...
class PredictionModel(nn.Module):
    ...
    def __init__(self,
        ...
        if weights_path is None:
            self.weights_path = os.path.join(CURRENT_DIR, "lstm_h256_ep6.pt")
        else:
            self.weights_path = weights_path

        if bias_path is None:
            self.bias_path = os.path.join(CURRENT_DIR, "delta_bias_h256_ep6.npy")
        else:
...
def train_model(model, feat, d1, raw, jump, epochs=6, seq_len=32, lr=1e-3, batch=64):
...
if __name__ == "__main__":
    ...
    weights_path = os.path.join(CURRENT_DIR, "lstm_h256_ep6.pt")
    bias_path = os.path.join(CURRENT_DIR, "delta_bias_h256_ep6.npy")
    ...
    if os.path.exists(weights_path) and os.path.exists(bias_path):
        ...
        model = PredictionModel(256, weights_path=weights_path, bias_path=bias_path)
        train_model(model, feat, d1, raw, jump, epochs=6)
