...
class PredictionModel(nn.Module):
    ...
    def __init__(self,
        ...
        if weights_path is None:
            self.weights_path = os.path.join(CURRENT_DIR, "lstm_h256_ep8.pt")
        else:
            self.weights_path = weights_path

        if bias_path is None:
            self.bias_path = os.path.join(CURRENT_DIR, "delta_bias_h256_ep8.npy")
        else:
...
if __name__ == "__main__":
    ...
    weights_path = os.path.join(CURRENT_DIR, "lstm_h256_ep8.pt")
    bias_path = os.path.join(CURRENT_DIR, "delta_bias_h256_ep8.npy")
    ...
    if os.path.exists(weights_path) and os.path.exists(bias_path):
        ...
        model = PredictionModel(256, weights_path=weights_path, bias_path=bias_path)
        train_model(model, feat, d1, raw, jump, epochs=8)
