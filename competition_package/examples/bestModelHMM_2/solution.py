...
class PredictionModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.warmup_steps = 8  # <<< warm-up before HMM scaling kicks in
        ...
        self.delta_buffer = []
        self.raw_buffer = []
        self.hmm_state_level = 0.0  # smoothed "regime" scalar
    ...
    def predict(self, dp: DataPoint):
        ...
        # volatility-scaled delta (input-side scaling)
        ...
        # if no prediction requested yet → just buffer
        ...
        # run through model
        delta_pred_t, _, jump_logit_t = self.forward(x)
        delta_pred = delta_pred_t.cpu().numpy().reshape(-1)
        jump_score = float(jump_logit_t.item())

        # ---------------------------------------------------------
        # HMM state smoothing (ONLINE STABILIZATION with WARM-UP)
        # ---------------------------------------------------------
        # convert jump score → pseudo "high-vol" probability
        p = 1.0 / (1.0 + np.exp(-jump_score))   # sigmoid

        # smooth this into a slow regime level
        self.hmm_state_level = 0.8 * self.hmm_state_level + 0.2 * p

        steps_seen = len(self.sequence_history)

        if steps_seen >= self.warmup_steps:
            # only start mixing HMM effect after warm-up
            alpha = np.clip((steps_seen - self.warmup_steps) / 20.0, 0.0, 1.0)

            # multiplier: slightly boost deltas in high-vol regime, damp in low-vol
            hmm_multiplier = 1.0 + 0.35 * (self.hmm_state_level - 0.5)
            scaled = delta_pred * hmm_multiplier

            # smooth blend between raw and HMM-adjusted prediction
            delta_pred = (1 - alpha) * delta_pred + alpha * scaled
        # else: during warm-up, we leave delta_pred unchanged

        # ---------------------------------------------------------
        # Apply bias AFTER HMM / warm-up adjustments
        # ---------------------------------------------------------
