...
class PredictionModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.gain_path = os.path.join(CURRENT_DIR, "delta_gain.npy")
        ...
        # Load gain if present
        if os.path.exists(self.gain_path):
            print(f"Loading gain from {self.gain_path}")
            self.gain = np.load(self.gain_path).astype(np.float32)
        else:
            self.gain = None
    ...
    def predict(self, dp: DataPoint):
        ...
        - predict next delta
        - return raw + adjusted_predicted_delta
        """
        ...
        # ---- amplitude-aware post-processing ----
        # 1) partial bias correction (0.5 * bias instead of full)
        if self.bias is not None:
            delta_pred_np = delta_pred_np - 0.5 * self.bias

        # 2) per-feature gain scaling
        if self.gain is not None:
            delta_pred_np = delta_pred_np * self.gain
...
def train_model(
    ...
    """
    Train the model on [raw, delta, rm] windows to predict next delta.
    Also computes:
      - per-feature bias: mean(pred - true)
      - per-feature gain: std(true) / std(pred)
    """
    model.train()
    model.to(DEVICE)
    ...
    preds_all = np.concatenate(preds_all, axis=0)  # (M, 32)
    trues_all = np.concatenate(trues_all, axis=0)  # (M, 32)

    # bias: mean(pred - true)
    bias = (preds_all - trues_all).mean(axis=0)  # (32,)

    # gain: std(true) / std(pred)
    eps = 1e-6
    pred_std = preds_all.std(axis=0)
    true_std = trues_all.std(axis=0)
    gain = true_std / (pred_std + eps)

    # clamp gain to avoid insane scaling
    gain = np.clip(gain, 0.5, 2.0).astype(np.float32)
    ...
    np.save(gain_path, gain.astype(np.float32))
    print("Saved bias correction vector to", bias_path)
    print("Saved gain vector to", gain_path)
...
if __name__ == "__main__":
    ...
    if os.path.exists(weights_path):
        ...
        model = PredictionModel()  # will load weights + bias + gain in __init__
    else:
        ...
        # Reload to ensure we're using clean loaded state + bias + gain
        model = PredictionModel()
