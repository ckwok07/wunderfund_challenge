...
# ============================================================
# MAIN — fixed hidden_size=256, sweep epochs = 4, 6, 8
# ============================================================
...
if __name__ == "__main__":
    ...
    for ep in epoch_list:
        print("\n" + "=" * 70)
        print(f"TRAINING / SCORING FOR EPOCHS = {ep}")
        print("=" * 70)

        weights_path = os.path.join(CURRENT_DIR, f"lstm_h256_ep{ep}.pt")
        bias_path = os.path.join(CURRENT_DIR, f"delta_bias_h256_ep{ep}.npy")

        if os.path.exists(weights_path) and os.path.exists(bias_path):
            print(f"⚡ Found existing ep{ep} weights — skipping training")
            ...
            print(f"🚀 No weights for ep{ep} — training now...")
            model = PredictionModel(
                hidden_size=256,
                weights_path=weights_path,
                bias_path=bias_path
        ...
        print(f"\n📊 RESULTS for epochs = {ep}")
        print(f"Mean R²: {results['mean_r2']:.6f}")
        print("First 5 features:")
        for i, f in enumerate(cols[:5]):
            print(f"{f}: {results[f]:.6f}")

    print("\n" + "=" * 60)
    print("Finished sweep. Best epoch = highest Mean R².")
    print("=" * 60)
