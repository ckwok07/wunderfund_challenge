...
class PredictionModel(nn.Module):
    ...
    def __init__(self,
                 hidden_size: int = 256,
                 input_dim: int = 192,  # 32 raw + 32 d1 + 32 rm_delta + 32 rm_raw + 32 norm + 32 rolling_std
    ...
    def predict(self, dp: DataPoint):
        ...
        # add rolling std
        feat = np.concatenate([raw, d1, rm_delta, rm_raw, norm_d1, vol_delta], axis=0)
...
def build_features_and_targets(df, cols, rm_window=5):
    ...
    feats = np.concatenate([raw, d1, rm_delta, rm_raw, norm, vol], axis=1)
